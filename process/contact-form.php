...
if(isset($_POST["action"])) {
	...
	$preference =	$_POST['preference']; // Sender's contact preference
	...
	$to =			"jeff@nr.net"; // Recipient's email address
	$subject =		"$website Contact Form Submission"; // Message title

	$body =			"From: $name \n E-Mail: $email \n Phone: $phone \n Reply Via: $preference \n Message: $message"  ;
	...
	if (($_POST['preference'] == "phone" || $_POST['preference'] == "either") && $_POST['phone'] == '') {
		$errmsg .= '<p>Please enter your phone number.</p>';
	}
	//Check if a contact method preference has been chosen
	if ($_POST['preference'] == '') {
		$errmsg .= '<p>Please select a preferred contact method.</p>';
	}
 ...
 */
function validation_google_captcha( $captcha_response){

	/* google captcha secret key*/
	$captcha_secret_key = '6LcA4fIaAAAAAEqo510K5iIHwLuw0b78oxZVLfWF';
...
//return(print_r($response));
	...
	if( isset($response['error-codes']) && !empty($response['error-codes'])){
		...
		}elseif( $response['error-codes'][0] == 'bad-POST' ){

			$error_message = '<p>The recaptcha POST is invalid or malformed.</p>';
		}
	}	
	if( $error_message !=''){
