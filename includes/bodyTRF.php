	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<span class="dropcap square large" style="background-color: #FFFFFF; padding: 5px 5px 60px 0px;"><img src="images/<?php echo $handicapper['thumbnail']; ?>" alt="<?php echo $fullname; ?>" title="<?php echo $fullname; ?>"></span><?php echo $handicapper['bio']; ?>
			</div>
...
<?php
	if ($handicapper['tracks'] == 0){  // Placeholder or otherwise not active w/o flagging as fully inactive...set their tracks value to 0
?>

 			<div class="modal-body">
				<span class="dropcap square large" style="background-color: #FFFFFF; padding: 5px 5px 60px 0px;"><img src="images/<?php echo $handicapper['thumbnail']; ?>" alt="<?php echo $fullname; ?>" title="<?php echo $fullname; ?>"></span>
...
?>
 			<div class="modal-body">
				<span class="dropcap square large" style="background-color: #FFFFFF; padding: 5px 5px 60px 0px;"><img src="images/<?php echo $handicapper['thumbnail']; ?>" alt="<?php echo $fullname; ?>" title="<?php echo $fullname; ?>"></span><h4 class="pt-2"><?php echo $handicapper['picks_slug']; ?></h4>
			</div>
...
<div class="modal-body">
	...
	<p align='center'><br><em>Ignore the man behind the curtain below - he's constructing</em><br><i class="fa fa-caret-down"></i>&nbsp;<i class="fa fa-caret-down"></i>&nbsp;<i class="fa fa-caret-down"></i>&nbsp;<i class="fa fa-caret-down"></i>&nbsp;<i class="fa fa-caret-down"></i></p>
</div>
					...
					<div class="col-lg-12 col-md-16">
						<div class="tab tab-border">
							<ul class="nav nav-tabs" id="myTab" role="tablist">
								<li class="nav-item">
									<a class="nav-link active show" id="all-tracks-tab" data-toggle="tab" href="#all-tracks" role="tab" aria-controls="all-tracks" aria-selected="true"><i></i> All Tracks </a>
								</li>
								<li class="nav-item">
									<a class="nav-link" id="one-track-tab" data-toggle="tab" href="#one-track" role="tab" aria-controls="one-track" aria-selected="false"> <i></i> One Track </a>
								</li>
								<li class="nav-item">
									<a class="nav-link" id="free-pick-tab" data-toggle="tab" href="#free-pick" role="tab" aria-controls="free-pick" aria-selected="false"><i></i> Free Pick </a>
								</li>
							</ul>
							...
							<div class="tab-content" id="myTabContent">
								<div class="tab-pane fade active show" id="all-tracks" role="tabpanel" aria-labelledby="all-tracks-tab">
									<div class="login-box-02 bg-light">
...
<?php
	...
	while($track = $rsTracks->fetch(PDO::FETCH_ASSOC)){
		...
		echo'
											<div class="col">
												<img src="images/'.$track['logo'].'" alt="'.$track['track'].'" title="'.$track['track'].'" width = "95%">
						                    </div>
		';
	}
?>
										...
										<div class="container">
											<div class="row justify-content-center">
												<div class="col mt-4 text-left">
													<p style="font-size: x-small;"><em>*Stated savings over Monthly subscription<br>&nbsp;All subscriptions will auto-renew. Cancel any time and your service will stop at the end of your current billing period.</em></p>
												</div>
											</div>
										</div>
									</div>
...
<!----------------------------------------------------------------------->
								<!-- One Track Tab -->
								<div class="tab-pane fade" id="one-track" role="tabpanel" aria-labelledby="one-track-tab">
									<div class="login-box-02 bg-light">

										<div class="container">
											<div class="row justify-content-center mb-3">
												<div class="col text-center">
													<h3>Select Your Favorite Track(s)</h3>
													<h5>Choose One Day's Picks or a Full Subscription</h5>
												</div>
											</div>
										</div>

<?php
	// Now get the tracks this handicapper handicaps again (PDO on MySQL won't allow the reuse of the first result set - no "CURSOR_SCROLL" (FIX THIS ORACLE!)
	$rsTracks->execute($aTValues);
	while($track = $rsTracks->fetch(PDO::FETCH_ASSOC)){
		echo'
										...
										<div class="row justify-content-around mt-4 border-bottom">
		';

		// Get the subscription types for this track
		$query = "
			SELECT *
			FROM subscription_types
			WHERE tracks = :tracks
		...
		while($sub = $rsSubTypes->fetch(PDO::FETCH_ASSOC)){
			echo'
											<div class="col text-center">
												<button type="button" class="btn btn-secondary btn-sm btn-block" value="'.$sub['uid'].'">'.$sub['period'].'<br>$'.$sub['rate'].'</button>
												<p class="fs-6" style="color: #FF0000; font-weight: bold;">'.$sub['savings'].'</p>
						    	            </div>
			';
		}
...
?>
										...
										<div class="container">
											<div class="row justify-content-center">
												<div class="col mt-4 text-left">
													<p style="font-size: x-small;"><em>*Stated savings over Monthly subscription<br>&nbsp;All non-daily subscriptions will auto-renew. Cancel any time and your service will stop at the end of your current period.</em></p>
												</div>
											</div>
										</div>
									</div>
...
<!----------------------------------------------------------------------->
								<!-- Free Pick Tab -->
								<div class="tab-pane fade" id="free-pick" role="tabpanel" aria-labelledby="free-pick-tab">
									<div class="login-box-02 bg-light">

										<div class="container">
											<div class="row justify-content-center mb-3">
												<div class="col text-center">
													<h3>FREE SELECTION</h3>
													<h5>Enjoy!!</h5>
												</div>
											</div>
										</div>
...
<?php
	...
	// Get the most recent published free pick and accompanying info
	$query = "
		SELECT *
		FROM free_picks p, handicappers h, tracks t
		WHERE h.uid = p.handicapper
		AND t.uid = p.track
		AND p.publish = :publish
		ORDER BY p.date DESC
		LIMIT 1
	";
	try{
		$rsFreePick = $dbLink->prepare($query);
		$aValues = array(
			':publish'	=> "1"
		);
		//$preparedQuery = str_replace(array_keys($aValues), array_values($aValues), $query); //Debug
		//echo $preparedQuery;//die();
		$rsFreePick->execute($aValues);
	}
	catch(PDOException $error){
		// Log any error
	//	file_put_contents($_SESSION['pdo_log'], "-----\rDate: ".date('Y-m-d H:i:s')."\rFile: ". __FILE__ ."\rLine Number: ". __LINE__ ."\rVars:\r".dump_vars(get_defined_vars())."\r", FILE_APPEND);
	...
	while($pick = $rsFreePick->fetch(PDO::FETCH_ASSOC)){
		echo'
										<div class="row align-items-end" style="height: 100px;">
											<div class="col-3">
												<img src="images/'.$pick['logo'].'" alt="'.$pick['track'].'" title="'.$pick['track'].'" width = "95%">
					    	                </div>
											<div class="col-9" align="right">
												<h4>'.$pick['track'].'</h4>
												<h6>'.$pick['location'].'</h6>
												<h6>'.date('l, F jS, Y', strtotime($pick['date'])).'</h6>
					    	                </div>
										</div>

										<div class="row justify-content-around mt-4 border">
											<div class="col text-left" style="background-color: #f3f3f3;">
												<p class="fs-6"><pre class="fs-6">'.$pick['pick'].'</pre></p>
						    	            </div>
										</div>
										...
										<div class="container">
											<div class="row justify-content-center">
												<div class="col mt-4 text-center">
													<p style="font-size: small;">This free selection is proudly sponsored by</p>
													<a href="https://'.$pick['sponsor_url'].'" target="_blank" title="'.$pick['sponsor_tagline'].'" onmouseover="this.style.fontWeight=\'bold\';" onmouseout="this.style.fontWeight=\'normal\';" style="font-size: x-large; color: #FF0000;">'.$pick['sponsor'].'</a><p style="font-size: small; color: #FF0000;">'.$pick['sponsor_tagline'].'</p>
													<p style="font-size: small;">and provided each racing day for races in California or major stakes races around the US</p>
												</div>
											</div>
										</div>
									</div>
...
?>
			<div class="col-lg-2" style="font-size: 14px; color: #000000;">
				...
				<img src="images/<?php echo $handicapper['picture']; ?>" alt="<?php echo $fullname; ?>"><p class="pb-2" style="font-size: 16px"><?php echo $fullname; ?></p></a>
			</div>
...
?>
		</div> <!-- This </div> seems orphaned but it is not - leave it here. -->
	</div>
</section>
